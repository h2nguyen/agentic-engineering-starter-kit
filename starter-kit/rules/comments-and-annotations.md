...
| Installer block markers | `# >>> starter-kit registries >>>` and `# <<< starter-kit registries <<<` in a `.gitattributes` the installer appended to | `bootstrap.sh` writes them when it appends its block to a pre-existing file and looks for them on re-run to skip a second append; a freshly copied template carries none |
